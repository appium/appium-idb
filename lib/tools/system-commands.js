  ...
  try {
    ...
    await new B((resolve, reject) => {
      for (const outType of ['stderr', 'stdout']) {
        ...
        companionProc.on(`lines-${outType}`, _.last(listeners));
      }

      listeners.push((code, signal) => {
        listeners.map((l) => companionProc.removeListener(l));
        listeners = [];
        const message = `${IDB_COMPANION_EXECUTABLE} exited with code '${code}' from signal '${signal}'`;
        log.debug(message);
        reject(new Error(message));
      });
      companionProc.once('exit', _.last(listeners));
