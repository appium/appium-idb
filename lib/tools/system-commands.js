    ...
    try {
      await waitForCondition(async () => {
        try {
          ...
          if (daemon.isRunning) {
            log.info(`${IDB_EXECUTABLE} daemon is running on port ${this.executable.port || DEFAULT_IDB_PORT}`);
          } else {
            if (!daemonOutput.includes('address already in use')) {
              throw new Error(`${IDB_EXECUTABLE} daemon has failed to start: ${daemonOutput}`);
            }
            log.info(`The port ${this.executable.port || DEFAULT_IDB_PORT} is already in use. ` +
