...
systemCallMethods.connect = async function connect () {
  ...
  const connectFunc = async () => {
    ...
    // sometimes idb needs more time to stabilize the connection
    await retryInterval(3, 300, async () => await tpExec(IDB_EXECUTABLE, ['describe', '--udid', this.udid]));
  };
  try {
    await retryInterval(2, 1000, async () => {
      try {
        ...
        try {
          ...
          if (daemonError.stderr || daemonError.stdout) {
            log.debug(daemonError.stderr || daemonError.stdout);
          }
          throw daemonError;
        } finally {
