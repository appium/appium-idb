...
const COMPANION_PGREP_PATTERN = (udid) => `${IDB_COMPANION_EXECUTABLE}.*--udid[[:space:]]+${udid}`;
...
systemCallMethods.connect = async function connect(opts = {}) {
  ...
  for (const binary of [IDB_EXECUTABLE, IDB_COMPANION_EXECUTABLE]) {
    try {
      ...
      throw new Error(
        `'${binary}' has not been found in PATH. ` +
  ...
  try {
    ...
    throw new Error(
      `Cannot start ${IDB_EXECUTABLE} service for the device '${this.udid}'. ` +
        `Check the server log for more details.`,
    );
  }
  log.info(
    `Successfully established the connection to ${IDB_EXECUTABLE} service for '${this.udid}'`,
  );
...
systemCallMethods.waitForDevice = async function waitForDevice(timeoutMs = 10000) {
  ...
  try {
    ...
    throw new Error(
      `The device '${this.udid}' is not responding to idb requests after ${timeoutMs}ms timeout. ` +
        `Original error: ${lastError || e.message}`,
    );
  }
  log.debug(
    `The device '${this.udid}' is online and ready to accept idb commands in ` +
...
systemCallMethods.disconnect = async function disconnect() {
  ...
  log.debug(
    `Cleaning up ${companionPids.length} obsolete ${IDB_COMPANION_EXECUTABLE} ` +
...
systemCallMethods.exec = async function exec(cmd, args = [], opts = {}) {
  ...
  try {
    ...
    if (util.hasValue(e.code)) {
      e.message =
        `Error executing ${IDB_EXECUTABLE}. Original error: '${e.message}'; ` +
