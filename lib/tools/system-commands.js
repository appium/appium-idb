...
systemCallMethods.connect = async function connect () {
  ...
  try {
    ...
    if (_.isEmpty(await getPids(COMPANION_PGREP_PATTERN(this.udid)))) {
      ...
      comp.on('output', (stdout, stderr) => {
        if (isStartupMonitorEnabled && _.trim(stdout || stderr)) {
          companionOutput += `[${IDB_COMPANION_EXECUTABLE}] ${stdout || stderr}`;
        }
      });
      try {
      ...
      await B.delay(300);
      if (!comp.isRunning) {
        throw new Error(`${IDB_COMPANION_EXECUTABLE} has failed to start: ${companionOutput}`);
      }
    ...
    if (daemon.isRunning) {
      log.info(`${IDB_EXECUTABLE} daemon is running on port ${this.executable.port}`);
    } else {
      if (!daemonOutput.includes('address already in use')) {
        throw new Error(`${IDB_EXECUTABLE} daemon has failed to start: ${daemonOutput}`);
      }
      log.info(`${this.executable.port} is already in use. Assuming it is used by ${IDB_EXECUTABLE} daemon`);
    }
    ...
    try {
      ...
      if (companionOutput) {
        log.debug(companionOutput);
      }
      if (daemonOutput) {
        log.debug(daemonOutput);
      }
