...
function buildDaemonArgs (opts = {}) {
  ...
  if (logPath) {
    result.push('--log-file-path', '/tmp/companion_log');
  }
  if (terminateOffline) {
    result.push('--terminate-offline', terminateOffline);
  }
  return result;
}
...
systemCallMethods.connect = async function connect (opts = {}) {
  ...
  const binaryPaths = {};
  for (const binary of [IDB_EXECUTABLE, IDB_COMPANION_EXECUTABLE]) { //eslint-disable-line
    try {
      ...
      await retryInterval(2, 100, async () => {
        ...
        try {
          ...
          daemon.on('output', (stdout, stderr) => {
            ...
            log.debug(`[daemon] ${stdout || stderr}`);
          });
          try {
