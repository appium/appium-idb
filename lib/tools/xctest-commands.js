 ...
 * @param {string} testType The type of test. Can be one of ['app', 'ui', 'logic']
...
xctestCommands.runXCUITest = async function runXCUITest (
  testRunnerBundleId, appUnderTestBundleId, xctestBundleId, testType = 'ui', opts = {}
) {
  // This maintains backwards compatibility for when testType wasn't
  // a parameter
  if (_.isObject(testType)) {
    opts = testType;
    testType = 'ui';
  }
  const uiTestProcess = this.createSubProcess([
    'xctest', 'run',
    testType, xctestBundleId,
...
xctestCommands.listXCTestBundles = async function listXCTestBundles () {
  ...
  try {
    let bundles = fixOutputToArray(output);
