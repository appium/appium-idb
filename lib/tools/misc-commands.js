 ...
 * architecture: "x86_64"
 *
 * @returns {object} The following information is returned:
...
miscCommands.clearKeychain = async function clearKeychain () {
  log.debug(`Clearing keyvhain on the device '${this.udid}'`);
  await this.exec(['clear_keychain']);
};
...
miscCommands.setLocation = async function setLocation (latitude, longitude) {
  ...
  await this.exec(['set_location', latitude, longitude]);
};
